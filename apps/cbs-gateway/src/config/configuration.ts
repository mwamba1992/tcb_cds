/** Configuration for the cbs-gateway service. Validated at boot: a bad value stops the process. */

export interface CbsGatewayConfig {
  nodeEnv: string;
  port: number;
  databaseUrl: string;
  jwt: {
  ...
  cbs: {
    /** `stub` until TCB's Core Banking test environment is available. */
    mode: 'stub' | 'live';
    baseUrl?: string;
  };
}
...
export function loadConfig(): CbsGatewayConfig {
  const nodeEnv = process.env['NODE_ENV'] ?? 'development';
  const rs256 = (process.env['JWT_ALGORITHM'] ?? 'HS256') === 'RS256';

  const config: CbsGatewayConfig = {
    nodeEnv,
    port: optionalNumber('CBS_GATEWAY_PORT', 3105),
    databaseUrl: required('CBS_GATEWAY_DATABASE_URL'),
    jwt: {
    ...
    cbs: {
      mode: optional('CBS_MODE') === 'live' ? 'live' : 'stub',
      baseUrl: optional('CBS_BASE_URL'),
    },
  };
  ...
  if (nodeEnv === 'production') {
    ...
    if (config.cbs.mode !== 'live' || !config.cbs.baseUrl) {
      throw new ConfigError('CBS_MODE must be live, with CBS_BASE_URL set, in production');
    }
  }
  return config;
}

export const CONFIG = 'CBS_GATEWAY_CONFIG';
