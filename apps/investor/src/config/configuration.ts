/** Configuration for the investor service. Validated at boot: a bad value stops the process. */
...
export function loadConfig(): InvestorConfig {
  const nodeEnv = process.env['NODE_ENV'] ?? 'development';
  const rs256 = (process.env['JWT_ALGORITHM'] ?? 'HS256') === 'RS256';

  const config: InvestorConfig = {
    nodeEnv,
    port: optionalNumber('INVESTOR_PORT', 3102),
    databaseUrl: required('INVESTOR_DATABASE_URL'),
    jwt: {
...
export const CONFIG = 'INVESTOR_CONFIG';
